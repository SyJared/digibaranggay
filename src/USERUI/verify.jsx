...
function Verify({ isOpen, onClose, onSubmit }) {
  ...
  const handleChange = (value, index) => {
    if (!/^[0-9]?$/.test(value)) return; // only 1 digit allowed
    const newPin = [...pin];
    newPin[index] = value;
    setPin(newPin);
  ...
  const handleSubmit = async () => {
    const fullPin = pin.join("").padStart(4, "0"); // ensures "0441" format
    ...
    const result = await onSubmit(fullPin); // send as string
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl p-8 w-[380px] shadow-2xl text-center space-y-6">
        <h2 className="text-xl font-bold text-gray-800">Enter 4-Digit PIN</h2>
        ...
        <div className="flex justify-center gap-8">
          {pin.map((digit, index) => (
            <input
              ...
              className={`w-12 text-center text-3xl font-semibold bg-transparent border-b-2 ${
                error ? "border-rose-500" : "border-gray-400"
              } focus:border-teal-800 focus:outline-none transition-all duration-200`}
            />
          ))}
        </div>
