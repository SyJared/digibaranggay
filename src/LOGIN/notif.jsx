...
export default function NotificationBell() {
  ...
  async function fetchNotifications() {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/digidb/get_notifications.php`, {
        credentials: "include",
      });
  ...
  async function markAllAsRead(list) {
    ...
    await Promise.all(
      list.map((n) =>
        fetch(`${import.meta.env.VITE_API_URL}/digidb/mark_notification_read.php`, {
          method: "POST",
          credentials: "include",
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {open && (
        <div className="fixed top-16 right-4 w-[90vw] max-w-sm sm:max-w-md md:max-w-lg bg-white shadow-xl rounded-xl border z-50">
          {/* Header */}
